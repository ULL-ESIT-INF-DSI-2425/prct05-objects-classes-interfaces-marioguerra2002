...
describe("Pokemon class tests", () => {
  ...
  test ("Should search a Pokemon by type", () => {
    const pokedex = new Pokedex();
    const pikachu = new Pokemon("Pikachu", 6, 4, PokemonType.Electrico, 55, 40, 90, 35);
    const charmander = new Pokemon("Charmander", 8.5, 6, PokemonType.Fuego, 52, 43, 65, 39);
    const bulbasaur = new Pokemon("Bulbasaur", 6.9, 7, PokemonType.Hierba, 49, 49, 45, 45);
    // an electric pokemon to see 2 pokemons in the array
    const raichu = new Pokemon("Raichu", 30, 8, PokemonType.Electrico, 90, 55, 110, 60);
    pokedex.addPokemon(pikachu);
    pokedex.addPokemon(charmander);
    pokedex.addPokemon(bulbasaur);
    pokedex.addPokemon(raichu);
    expect(pokedex.searchByType(PokemonType.Electrico)).toEqual([pikachu, raichu]);
  });
  ...
  test ("Should throw an error if the Pokemon already exists", () => {
    ...
    const pikachu = new Pokemon("Pikachu", 6, 4, PokemonType.Electrico, 55, 40, 90, 35);
    pokedex.addPokemon(pikachu);
    expect(() => pokedex.addPokemon(pikachu)).toThrowError('Pokemon already exists');
  });
...
describe ("Combat class tests", () => {
  ...
  // pokemon tipo agua
  const squirtle = new Pokemon("Squirtle", 9, 6, PokemonType.Agua, 44, 48, 65, 43);
  ...
  test ("Given two pokemons, in case getEfectiveness does not find the value, return 1", () => {
    const invalidPokemon = new Pokemon("Invalid", 6, 4, PokemonType.Inventado, 55, 40, 90, 35);
    ...
    expect(() => combat.calculateDamage(pikachu, invalidPokemon)).toThrowError('No se ha encontrado la efectividad');
  });
  test ("Start the combat between two pokemons", () => {
    const combat = new Combat(pikachu, charmander);
    const consoleSpy = vi.spyOn(console, 'log'); // spy on console.log
    combat.start();
    expect(consoleSpy).toHaveBeenCalledWith("Comienza el combate entre Pikachu y Charmander");
    expect(consoleSpy).toHaveBeenCalledWith("Pikachu ataca a Charmander y le hace 63.95 de daño");
  ...
  test ("Should calculate the damage between two pokemons", () => {
    ...
    const consoleSpy = vi.spyOn(console, 'log'); // spy on console.log
    combat.start();
    expect(consoleSpy).toHaveBeenCalledWith("Comienza el combate entre Pikachu y Bulbasaur");
    expect(consoleSpy).toHaveBeenCalledWith("Pikachu ataca a Bulbasaur y le hace 56.12 de daño");
    expect(consoleSpy).toHaveBeenCalledWith("Bulbasaur tiene 53.88 de vida");
    expect(consoleSpy).toHaveBeenCalledWith("Bulbasaur ataca a Pikachu y le hace 61.25 de daño");
